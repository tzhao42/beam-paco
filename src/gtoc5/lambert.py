...
class lambert_eval(object):
    @expand_kwargs
    def __init__(
        self, leg_dT, dep_ast, arr_ast, dep_t, dep_m, *args, **kwargs
    ):
        self.dT = leg_dT
...
def lambert_optimize_dt(
    ...
    **kwargs
    ...
    """
    assert (leg_dT_bounds is None) ^ (
        leg_dT is None
    ), "One (and only one) of leg_dT_bounds or leg_dT must be specified."
    assert not random_pareto or (
        random_pareto and grid
    ), "random_pareto=True requires grid=True."
