...
def seq(mission, incl_flyby=True):
    ...
    return [mission[0][0]] + [l[0] for l in mission[1:]][
        :: 1 if incl_flyby else 2
    ]
...
def add_asteroid(mission, next_ast, use_cache=True, stats=None, **kwargs):
    ...
    assert (
        isinstance(next_ast, (int, np.integer)) and 0 < next_ast <= 7075
...
def rendezvous_leg(
    ...
    **kwargs
    ...
    dep_ast, dep_m, dep_t = mission[-1][:3]
    leg = lambert_optimize_dt(
        dep_ast,
        next_ast,
        dep_t,
        dep_m,
        leg_dT=leg_dT,
        leg_dT_bounds=leg_dT_bounds,
        obj_fun=obj_fun,
        mission=mission,
        stats=stats,
        **kwargs
...
class gtoc5_rendezvous_agg(gtoc5_rendezvous):
    ...
    def inspect(self, mission=None, *args, **kwargs):
        assert mission is not None, (
            "Unknown `mission`. " "Resource rating cannot be calculated"
        )
        ...
        if self.feasible:
            ...
            self.resource_rating = rate_traj(
                self.arr_m, tof(mission) + self.dT
            )
