...
class traj_stats(object):
    ...
    def log_mission(self, m):
        ...
        t = (
            ...
            self.nr_traj,
            self.nr_legs,
            self.nr_legs_distinct,
            self.nr_legs_feasible,
            self.nr_lambert,
        )
    ...
    def pareto_front_evolution(
        self, every_nr_legs=1000, up_to_nr_legs=100000, if_score=16
    ):
        """
...
class gtoc5_ant(object):
    ...
    def __init__(
        ...
        self.rate_destinations = (
            rate__orbital_2 if ratingf is None else ratingf
        )
    ...
    def initialize(self, aco):
        ...
        self.add_ast_args[
            "random"
        ] = aco.random  # needed if random_pareto=True
        # reset the caches
...
class gtoc5_ant_pareto(gtoc5_ant):
    ...
    def sort(self, evaluated_paths, r=None, f=None):
        ...
        * `f` (front threshold): returns only the top `f` Pareto fronts.
        ...
        objs = [
            [q[1:] for (q, p) in score_bin] for score_bin in evaluated_paths
        ]
